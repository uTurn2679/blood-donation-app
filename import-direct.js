...
let added = 0;
const insertUser = db.prepare('INSERT INTO User (id, name, phone, password, role, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?)');
const insertProfile = db.prepare('INSERT INTO DonorProfile (id, userId, bloodGroup, department, session, isAvailable, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?)');
const findUser = db.prepare('SELECT id FROM User WHERE phone = ?');

db.transaction(() => {
  for (const row of rawData) {
    const existing = findUser.get(row.Phone);
    if (!existing) {
      const userId = crypto.randomUUID();
      const profileId = crypto.randomUUID();
      const date = new Date().toISOString();
      
      insertUser.run(userId, row.Name, row.Phone, 'default_password', 'DONOR', date, date);
      insertProfile.run(
        profileId,
        userId,
        row.BloodGroup,
...
console.log('Inserted ' + added + ' users successfully via better-sqlite3 directly.');
