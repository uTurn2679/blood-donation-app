...
import { CheckCircle, XCircle, Clock, Award, ArrowLeft, FileCheck, MessageSquare } from "lucide-react";
...
export default async function ExamResultPage({
  ...
  const result = await getSubmissionDetailsAction(submissionId);
  const submission = result.submission;

  if (!result.success || !submission) {
    return (
      <div className="container" style={{ padding: "4rem 0" }}>
        <div style={{ background: "#fef2f2", padding: "2rem", borderRadius: "1rem", textAlign: "center" }}>
      ...
      <div className="container" style={{ maxWidth: "800px" }}>
        ...
        >
          ...
          <h1 style={{ fontSize: "2rem", fontWeight: 800, color: "#0f172a", marginBottom: "0.25rem" }}>
            {exam.title} - Answer Paper Result
          </h1>
          <p style={{ color: "#64748b", fontSize: "0.95rem", marginBottom: "1.5rem" }}>
            Student: <strong style={{ color: "#0f172a" }}>{submission.studentName}</strong>
          </p>

          <div style={{ display: "inline-flex", alignItems: "center", gap: "0.5rem", background: isGraded ? (isPassed ? "#dcfce7" : "#fee2e2") : "#fef3c7", color: isGraded ? (isPassed ? "#166534" : "#991b1b") : "#b45309", padding: "0.4rem 1.25rem", borderRadius: "9999px", fontWeight: 700, fontSize: "0.9rem", marginBottom: "2rem" }}>
            {isGraded ? (isPassed ? "PASSED ASSESSMENT" : "NEEDS IMPROVEMENT") : "UNDER TEACHER REVIEW"}
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "1rem", background: "#f8fafc", padding: "1.5rem", borderRadius: "1rem", border: "1px solid #f1f5f9" }}>
            <div>
              <span style={{ fontSize: "0.75rem", fontWeight: 700, color: "#64748b", textTransform: "uppercase" }}>YOUR SCORE</span>
            ...
            <div>
              <span style={{ fontSize: "0.75rem", fontWeight: 700, color: "#64748b", textTransform: "uppercase" }}>PASS MARK</span>
              <div style={{ fontSize: "1.75rem", fontWeight: 800, color: "#0f172a" }}>
                {exam.passMarks}
              </div>
            </div>
            <div>
              <span style={{ fontSize: "0.75rem", fontWeight: 700, color: "#64748b", textTransform: "uppercase" }}>TIME TAKEN</span>
              <div style={{ fontSize: "1.75rem", fontWeight: 800, color: "#0f172a" }}>
          ...
          {submission.feedback && (
            <div style={{ marginTop: "1.5rem", background: "#eff6ff", border: "1px solid #bfdbfe", padding: "1.25rem", borderRadius: "0.75rem", textAlign: "left" }}>
              <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", color: "#1d4ed8", fontWeight: 700, marginBottom: "0.35rem" }}>
        ...
        <h3 style={{ fontSize: "1.25rem", fontWeight: 700, color: "#0f172a", marginBottom: "1.25rem" }}>
          Question Breakdown & Answer Paper Details
        </h3>
            ...
            return (
              ...
              >
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "0.75rem" }}>
                  ...
                  <span style={{ fontSize: "0.85rem", fontWeight: 700, color: isCorrect ? "#16a34a" : isCorrect === false ? "#dc2626" : "#b45309" }}>
                ...
                {q.questionType === "MCQ" ? (
                  ...
                  <div style={{ marginTop: "0.75rem", background: "#f8fafc", padding: "0.85rem", borderRadius: "0.5rem", fontSize: "0.9rem" }}>
                    <span style={{ display: "block", fontSize: "0.75rem", fontWeight: 700, color: "#64748b", marginBottom: "0.25rem" }}>
                      SUBMITTED WRITTEN RESPONSE:
                    </span>
                    <p style={{ color: "#334155", fontStyle: studentAns?.writtenAnswer ? "normal" : "italic" }}>
                      {studentAns?.writtenAnswer || "No answer submitted."}
                    </p>
                  </div>
                )}
              </div>
